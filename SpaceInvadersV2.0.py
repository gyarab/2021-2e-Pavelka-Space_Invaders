...
class Nepritel(pygame.Rect):
    ...
    def zasah(a):
        global Skore, StrelaY, StrelaRychlost, StatusStrely
    ...
    def Reset(a):
        a.x = random.randint(1, Okno_Sirka - 5)
        a.y = 50


N1 = Nepritel(50, 50, 50)
def Vykreslit():
    Okno.blit(nepritel, N1)
...
def Prohra():
    global N1, Prohrals, ZnovuPos, NejlepsiSkore
    if N1.y >= Okno_vyska - 100 and N1.y <= Okno_vyska:
        N1.rychlost = 0
        HudbaProhry()
        Prohrals = True
        ZnovuPos = 250
...
while (Jede):
    ...
    if Scena == "hra":
        ...
        Nej_Text = SkoreStyl.render("Nejlepsší skóre: " + str(NejlepsiSkore), True, (255, 255, 255))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if Zpet_Button.collidepoint(mouse_pos):
                    ...
                    Scena = "menu"
                    HudbaVMenu()
                if Znovu_Button.collidepoint(mouse_pos):
                    HudbaVeHre()
                    Prohrals = False
                    N1.Reset()
                    RaketkaX = (Okno_Sirka / 2) - 50
